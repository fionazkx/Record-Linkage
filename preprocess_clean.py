...
df_train['question1'] = df_train.question1.map(lambda x: ' '.join(
            [stemmer.stem(word) for word in
             nltk.word_tokenize(clean_text(str(x).lower()))]))
df_train['question2'] = df_train.question2.map(lambda x: ' '.join(
    [stemmer.stem(word) for word in
     nltk.word_tokenize(clean_text(str(x).lower()))]))

df_test['question1'] = df_test.question1.map(lambda x: ' '.join(
            [stemmer.stem(word) for word in
             nltk.word_tokenize(clean_text(str(x).lower()))]))
df_test['question2'] = df_test.question2.map(lambda x: ' '.join(
    [stemmer.stem(word) for word in
     nltk.word_tokenize(clean_text(str(x).lower()))]))
...
# get the rate of unique questions in total questions
questions = df_train['question1'].tolist() + df_train['question2'].tolist()+df_test['question1'].tolist()+df_test['question2'].tolist()
len_questions = len(questions)
len_uniq_questions = len(set(questions))
unique_rate =  1.0 * len_uniq_questions / len_questions


# output
df_train.question1 = df_train.question1.map(lambda x: ' '.join(x.split()))
df_train.question2 = df_train.question2.map(lambda x: ' '.join(x.split()))
df_test.question1 = df_test.question1.map(lambda x: ' '.join(x.split()))
